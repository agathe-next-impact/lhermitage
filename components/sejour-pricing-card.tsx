...
export function SejourPricingCard({ sejour, featured = false }: SejourPricingCardProps) {
  if (!sejour || !sejour.id) {
    console.error("[v0] Invalid sejour object passed to SejourPricingCard")
    return null
  }
  ...
  const getTitle = (item: any): string => {
    try {
      ...
      console.error("[v0] Error getting title:", error)
    }
    return "Sans titre"
  }
  ...
  const getSlug = (item: any): string | null => {
    try {
      ...
      console.error("[v0] Error getting slug:", error)
      return null
    }
  }
  ...
  const getDescriptionExcerpt = (html: string | undefined, maxLength: number = 150): string => {
    try {
      ...
      console.error("[v0] Error getting description excerpt:", error)
      return ""
    }
  }
