...
export function SejoursHeader({
  ...
  return (
    <div className="w-[calc(100%-1rem)] mt-3 mx-auto relative pt-16">
      {/* Grille hero — même structure que la homepage */}
      <section className="relative h-[calc(100vh-82px)] grid grid-cols-4 grid-rows-5">
        {/* Fond vidéo / image — ne dépasse pas la boîte titre (3 colonnes sur 4) */}
        <div
        ...
        {/* Encoche logo — haut gauche (positionnée en absolute comme PageHeader) */}
        ...
        {/* Encoche titre — haut droite (positionnée en absolute comme PageHeader) */}
        <div
        ...
        >
          ...
          >
            <h1 className="font-sans text-sm sm:text-base md:text-xl lg:text-2xl xl:text-4xl text-white leading-tight drop-shadow-sm">
              {title}
            </h1>
          </motion.div>
        ...
        {(accroche || sousTitre || ctaTexte) && (
          <div
            className="absolute right-0 z-10 flex flex-col justify-end p-5 md:p-6"
          ...
          >
            {accroche && (
              <motion.h2
                ...
                className="font-sans text-base md:text-xl lg:text-2xl xl:text-3xl text-brand-green leading-tight mb-3"
              >
                {accroche}
              </motion.h2>
            )}
            {sousTitre && (
              <motion.p
                ...
                className="text-sm md:text-base text-brand-gray leading-relaxed mb-4 drop-shadow-sm"
              >
                {sousTitre}
              </motion.p>
            )}
            {ctaTexte && (
              ...
              >
                <a
                  ...
                  className="inline-block px-6 py-3 rounded-full text-sm md:text-base font-semibold text-white transition-all hover:scale-105 hover:brightness-110 shadow-lg"
                  style={{ backgroundColor: mainBlobColor }}
                >
                  {ctaTexte}
                </a>
