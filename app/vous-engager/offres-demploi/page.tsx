...
const PAGE_PATH = "vous-engager/offres-demploi"

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageByPath(PAGE_PATH)
  const title = page?.title?.rendered || "Offres d'emploi"
  const description =
    page?.acf?.hero?.["sous-titre"] || "Decouvrez nos offres d'emploi a L'Hermitage"
...
export default async function OffresEmploiPage() {
  ...
  const heroTitle = page.title?.rendered || "Offres d'emploi"
