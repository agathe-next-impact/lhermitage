...
const PAGE_PATH = "vous-engager/services-civique"

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageByPath(PAGE_PATH)
  const title = page?.title?.rendered || "Services civique"
  const description =
    page?.acf?.hero?.["sous-titre"] || "Decouvrez nos offres de services civique a L'Hermitage"
...
export default async function ServicesCiviquePage() {
  const page = await getPageByPath(PAGE_PATH)
  if (!page) notFound()

  const recrutement = page.acf?.recrutement
  const heroTitle = page.title?.rendered || "Services civique"
