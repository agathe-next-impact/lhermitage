...
const PAGE_PATH = "vous-engager/pass-permis"

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageByPath(PAGE_PATH)
  const title = page?.title?.rendered || "Pass Permis"
  const description =
    page?.acf?.hero?.["sous-titre"] || "Decouvrez le programme Pass Permis a L'Hermitage"
...
export default async function PassPermisPage() {
  const page = await getPageByPath(PAGE_PATH)
  if (!page) notFound()

  const recrutement = page.acf?.recrutement
  const heroTitle = page.title?.rendered || "Pass Permis"
