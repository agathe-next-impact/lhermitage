...
const PAGE_PATH = "vous-engager/stages"

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageByPath(PAGE_PATH)
  const title = page?.title?.rendered || "Stages"
  const description =
    page?.acf?.hero?.["sous-titre"] || "Decouvrez nos offres de stages a L'Hermitage"
...
export default async function StagePage() {
  ...
  const heroTitle = page.title?.rendered || "Stages"
