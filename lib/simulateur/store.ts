...
interface SimulateurActions {
  ...
  initDaysFromTemplate: (
    templateDays: ProgrammeJour[],
    totalDays: number,
    fallbackCreneaux: string[]
  ) => void
...
export const useSimulateurStore = create<SimulateurStore>()(
  persist(
    subscribeWithSelector((set) => ({
      ...
      initDaysFromTemplate: (templateDays, totalDays, fallbackCreneaux) =>
        set(() => {
          const days: DayProgram[] = []
          for (let i = 1; i <= totalDays; i++) {
            const templateDay = templateDays.find((d) => d.jour_numero === i)
            if (templateDay) {
              days.push({
                dayNumber: i,
                slots: templateDay.creneaux.map((c) => {
                  const defaults = CRENEAU_SLOT_DEFAULTS[c.creneau]
                  return {
                    id: generateSlotId(),
                    heure_debut: defaults?.heure_debut ?? "09:00",
                    type_creneau: defaults?.type_creneau ?? "activite",
                    label_personnalise:
                      c.label_personnalise || CRENEAU_LABELS[c.creneau] || c.creneau,
                    ...(c.activite_suggeree ? { activite_slugs: [c.activite_suggeree.slug] } : {}),
                    ...(c.espace_suggere ? { espace_slugs: [c.espace_suggere.slug] } : {}),
                    ...(c.service_suggere ? { service_slugs: [c.service_suggere.slug] } : {}),
                  }
                }),
              })
            } else {
              days.push(createDayFromCreneaux(i, fallbackCreneaux))
            }
          }
          return { days }
        }),
